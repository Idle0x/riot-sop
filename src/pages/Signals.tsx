...
import { Search, Plus, Trash2, ArrowRight, Clock, CheckCircle2, AlertOctagon } from 'lucide-react';
import { Project, ProjectStatus } from '../types';
import { cn } from '../utils/cn';

export const Signals = () => {
  const { projects, addProject, updateProjectStatus } = useFinancials();
  ...
  const handleAddProject = () => {
    ...
    addProject({
      ...
      name: newName,
      type: newType,
      status: 'discovery', // Always start in Discovery
      description: newDesc,
      redFlags: [],
      greenFlags: [],
        ...
        <GlassCard className="p-6 border-accent-info/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="flex gap-2">
              <button 
                onClick={() => setNewType('hunter')}
                className={cn(
                  "flex-1 rounded-xl border font-bold transition-all",
                  newType === 'hunter' 
                    ? "bg-accent-info/20 border-accent-info text-accent-info" 
                    : "border-glass-border text-gray-500 hover:border-white/20"
                )}
              >
                HUNTER
              </button>
              <button 
                onClick={() => setNewType('creator')}
                className={cn(
                  "flex-1 rounded-xl border font-bold transition-all",
                  newType === 'creator' 
          ...
          <textarea 
            className="w-full bg-black/20 border border-glass-border rounded-xl p-4 text-white placeholder:text-gray-600 focus:border-accent-info outline-none resize-none mb-4"
            rows={2}
            placeholder="Quick notes: VCs, GitHub activity, or immediate red flags..."
            value={newDesc}
            onChange={(e) => setNewDesc(e.target.value)}
          />

          <div className="flex justify-between items-center">
            <button className="text-xs text-accent-danger font-bold flex items-center hover:text-red-400">
              <Trash2 className="w-3 h-3 mr-1" />
              REJECT (DEAD)
            </button>
            <GlassButton onClick={handleAddProject} disabled={!newName}>
              Add to Discovery
            </GlassButton>
          </div>
        </GlassCard>
          ...
          <div key={col.id} className="flex flex-col h-full bg-white/5 rounded-2xl border border-glass-border">
            ...
            <div className="flex-1 p-3 space-y-3 overflow-y-auto">
              {projects.filter(p => p.status === col.id).map(project => (
                <div 
                  key={project.id} 
                  className="group relative bg-glass hover:bg-white/10 border border-glass-border rounded-xl p-4 transition-all hover:-translate-y-1 hover:shadow-lg cursor-grab active:cursor-grabbing"
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {/* Move Actions (Simple for now) */}
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {col.id !== 'discovery' && (
                        <button 
                          onClick={() => updateProjectStatus(project.id, columns[columns.findIndex(c => c.id === col.id) - 1].id)}
