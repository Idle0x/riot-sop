              ...
              <div className="w-full max-w-md bg-zinc-900 border border-red-500/50 rounded-2xl p-6 shadow-[0_0_50px_rgba(239,68,68,0.2)]">
                  ...
                  <div className="space-y-4 mb-8">
                      ...
                      <label className="flex items-start gap-3 p-3 bg-white/5 rounded-lg cursor-pointer border border-transparent hover:border-red-500/30 transition-colors">
                          ...
                          <span className="text-sm text-gray-300">Mandatory cooling period observed.</span>
                      </label>
                      <label className="flex items-start gap-3 p-3 bg-white/5 rounded-lg cursor-pointer border border-transparent hover:border-red-500/30 transition-colors">
                          ...
                          <span className="text-sm text-gray-300">Reviewed the Manifesto.</span>
                      </label>
                  </div>
          ...
          <GlassCard className="w-full max-w-md max-h-[70vh] flex flex-col relative">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2">
              {triageHistory.length === 0 ? (
                <div className="text-gray-500 text-sm text-center py-8">No recorded sessions.</div>
              ) : (
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="relative">
                ...
                <button onClick={handleFetchRate} disabled={isFetchingRate} className="absolute right-2 top-8 text-[10px] bg-white/10 px-2 py-1 rounded text-accent-info">
            ...
            <div className="space-y-4 pt-2">
                ...
                <div className="p-4 bg-slate-500/10 rounded-xl border border-slate-500/20">
                    <div className="flex justify-between mb-2">
                        ...
                        <button onClick={() => openManualTo('protocol_z')} className="text-[10px] flex items-center gap-1 text-slate-500 hover:text-white bg-white/5 px-2 py-1 rounded">
                ...
                <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                    <div className="flex justify-between mb-2">
                        ...
                        <button onClick={() => openManualTo('filters')} className="text-[10px] flex items-center gap-1 text-gray-500 hover:text-white bg-white/5 px-2 py-1 rounded">
                           <HelpCircle size={10}/> Strategies
                        </button>
                    </div>
                ...
                <div className="p-4 bg-blue-500/10 rounded-xl border border-blue-500/20">
                    <div className="flex justify-between mb-2">
                        ...
                        <button onClick={() => openManualTo('engine')} className="text-[10px] flex items-center gap-1 text-blue-400/50 hover:text-blue-200 bg-blue-500/5 px-2 py-1 rounded">
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className={`p-4 rounded-xl border ${isGenerosityLocked ? 'bg-red-500/5 border-red-500/30' : 'bg-white/5 border-white/10'}`}>
              <div className="flex justify-between mb-2">
                ...
                <span className={`text-xs font-bold ${isOverCap ? 'text-red-500' : 'text-gray-500'}`}>Cap: <Naira/>{formatNumber(dynamicGenCap)}</span>
              </div>
              {!isGenerosityLocked && (
                <GlassInput value={generosity} onChange={(e: any) => setGenerosity(e.target.value)} className="text-right" placeholder="0" />
              )}
            </div>
            ...
            <div className="bg-black/20 p-4 rounded-xl border border-white/10 h-64 overflow-y-auto relative">
              {isCritical && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center text-center p-4">
                  ...
                  <p className="text-sm text-red-400">Runway Critical (&lt;3m)</p>
                </div>
              )}
              <div className="flex justify-between items-center mb-4">
                 <h3 className="text-xs font-bold text-gray-400 uppercase">Goals</h3>
                 <button onClick={autoDistribute} className="text-xs text-accent-success flex items-center gap-1"><Wand2 size={12}/> Auto-Fill</button>
              </div>
              {goals.filter(g => !g.isCompleted).map(g => (
                <div key={g.id} className="mb-3">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-white">{g.title}</span>
                    <span className="text-gray-500"><Naira/>{formatNumber(g.targetAmount - g.currentAmount)} left</span>
                  </div>
                  <input 
                    type="number" 
                    disabled={isCritical}
            ...
            <div className="flex justify-between items-center pt-4 border-t border-white/10">
              <span className="text-gray-400 text-sm">Remaining (Holding)</span>
