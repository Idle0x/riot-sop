          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-fade-in">
              <div className="w-full max-w-md bg-zinc-900 border border-red-500/50 rounded-2xl p-6 shadow-[0_0_50px_rgba(239,68,68,0.2)]">
                  ...
                  <div className="space-y-4 mb-8">
                      <p className="text-gray-400 text-sm text-center">Protocol activated for capital preservation.</p>
                      <label className="flex items-start gap-3 p-3 bg-white/5 rounded-lg cursor-pointer border border-transparent hover:border-red-500/30 transition-colors">
                          ...
                          <span className="text-sm text-gray-300">Silence Protocol engaged. Absolute discretion confirmed.</span>
                      </label>
                      <label className="flex items-start gap-3 p-3 bg-white/5 rounded-lg cursor-pointer border border-transparent hover:border-red-500/30 transition-colors">
                          ...
                          <span className="text-sm text-gray-300">Cooling period observed. Action is strategic, not impulsive.</span>
                      </label>
                      <label className="flex items-start gap-3 p-3 bg-white/5 rounded-lg cursor-pointer border border-transparent hover:border-red-500/30 transition-colors">
                          ...
                          <span className="text-sm text-gray-300">Core Constitution reviewed. Alignment verified.</span>
                      </label>
                  </div>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="relative">
                <GlassInput label="Rate (NGN/USD)" type="number" value={rate} onChange={(e: any) => setRate(e.target.value)} className={rateError ? 'border-red-500/50' : ''}/>
            ...
            <div>
              ...
              <select className="w-full bg-black/20 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-white/30" value={selectedSignalId} onChange={(e) => setSelectedSignalId(e.target.value)}>
            ...
            <div className="space-y-4 pt-2">
                <div className="p-4 bg-slate-500/10 rounded-xl border border-slate-500/20">
                    <div className="flex justify-between mb-2">
                        ...
                        <button onClick={() => openManualTo('protocol_z')} className="text-[10px] flex items-center gap-1 text-slate-500 hover:text-white transition-colors bg-white/5 px-2 py-1 rounded">
                ...
                <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                    <div className="flex justify-between mb-2">
                        ...
                        <button onClick={() => openManualTo('filters')} className="text-[10px] flex items-center gap-1 text-gray-500 hover:text-white transition-colors bg-white/5 px-2 py-1 rounded">
                ...
                <div className="p-4 bg-blue-500/10 rounded-xl border border-blue-500/20">
                    <div className="flex justify-between mb-2">
                        ...
                        <button onClick={() => openManualTo('engine')} className="text-[10px] flex items-center gap-1 text-blue-400/50 hover:text-blue-200 transition-colors bg-blue-500/5 px-2 py-1 rounded">
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="flex items-center gap-4 border-b border-white/10 pb-4">
               ...
               <div>
                  ...
                  {isBigDrop && <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded border border-yellow-500/50 font-mono">⚠ BIG DROP MODE</span>}
               </div>
            </div>
            ...
            <div className={`p-4 rounded-xl border ${isGenerosityLocked ? 'bg-red-500/5 border-red-500/30' : 'bg-white/5 border-white/10'}`}>
              <div className="flex justify-between mb-2">
                ...
                {isGenerosityLocked ? <span className="text-xs font-bold text-red-500 flex items-center gap-1"><Lock size={12}/> LOCKED</span> : <span className="text-xs font-bold text-gray-500">Cap: <Naira/>{formatNumber(dynamicGenCap)}</span>}
              </div>
              {!isGenerosityLocked && <GlassInput value={generosity} onChange={(e: any) => setGenerosity(e.target.value)} className="text-right" placeholder="0" />}
            </div>
            <div className="bg-black/20 p-4 rounded-xl border border-white/10 h-64 overflow-y-auto relative">
              {isCritical && <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center text-center p-4"><Lock className="text-red-500 mb-2" size={32}/><h3 className="font-bold text-white">Allocation Locked</h3></div>}
              <div className="flex justify-between items-center mb-4"><h3 className="text-xs font-bold text-gray-400 uppercase">Goals</h3><button onClick={autoDistribute} className="text-xs text-accent-success hover:underline">Auto-Fill</button></div>
              {goals.filter(g => !g.isCompleted).map(g => (
                <div key={g.id} className="mb-3">
                  <div className="flex justify-between text-xs mb-1"><span className="text-white">{g.title}</span><span className="text-gray-500">Need: <Naira/>{formatNumber(g.targetAmount - g.currentAmount)}</span></div>
                  <input type="number" disabled={isCritical} className="w-full bg-black/40 border border-white/10 rounded p-2 text-white text-sm" placeholder="Allocation" value={allocations[g.id] || ''} onChange={(e) => setAllocations({...allocations, [g.id]: parseFloat(e.target.value) || 0})} />
                </div>
              ))}
            </div>
