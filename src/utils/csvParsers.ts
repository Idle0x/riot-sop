...
const classifyTransaction = (rawDescription: string, type: 'SPEND' | 'DROP', amount: number): { merchant: string, category: string } => {
  ...
  if (desc.includes('google')) {
      // Extracts the app name between "GOOGLE " and "Lagos", "NG", or the pipe "|"
  ...
  if (desc.includes('basic feeding')) return { merchant: 'Basic Feeding', category: 'Food & Dining' }; // Caught from your FirstBank logs
  ...
  if (desc.includes('paystack*') || desc.includes('flw*') || desc.includes('paystack') || desc.includes('flutterwave')) {
      ...
      extractedName = extractedName.replace(/[0-9]{6,}/g, '').trim(); // Remove IDs
      // Clean up casing
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Memory bank to track identical transactions on the exact same day
        ...
        for (let i = headerRowIndex + 1; i < rows.length; i++) {
          ...
          // PASS TYPE AND AMOUNT INTO THE CLASSIFIER
